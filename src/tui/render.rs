...
impl Render for (ChatMessage, MessageType) {
    fn render(self) -> Spans<'static> {
        let (msg, ty) = self;
        let sender = match ty {
            MessageType::Sent => Span::styled("<You> ", Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)),
            MessageType::Received => Span::styled("<Them> ", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
        };
