...
fn run_sender(message_receiver: Receiver<ChatMessage>) -> Result<(), RecvError> {
    let mut buffer: VecDeque<DNSMessage> = VecDeque::new();
...
fn poll_messages<A: ToSocketAddrs + Clone>(message_sender: Sender<ChatMessage>, target: A) -> Result<(), SendError<ChatMessage>> {
    ...
    loop {
        ...
        let mut stream = TcpStream::connect(target.clone()).expect("Couldn't connect to target DNS server");
